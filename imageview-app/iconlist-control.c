...
typedef struct _IconListControlRetrieveThumbnailsData IconListControlRetrieveThumbnailsData;
...
struct _IconListControlRetrieveThumbnailsData {
	int _state_;
	GAsyncResult* _res_;
	GSimpleAsyncResult* _async_result;
	IconListControl* self;
...
void icon_list_control_retrieve_thumbnails (IconListControl* self, GCancellable* cancellable, GAsyncReadyCallback _callback_, gpointer _user_data_);
void icon_list_control_retrieve_thumbnails_finish (IconListControl* self, GAsyncResult* _res_);
...
GType icon_list_control_col_get_type (void) {
	...
	if (G_UNLIKELY (icon_list_control_col_type_id == 0)) {
		static const GEnumValue values[] = {{ICON_LIST_CONTROL_COL_TEXT, "ICON_LIST_CONTROL_COL_TEXT", "text"}, {ICON_LIST_CONTROL_COL_FILE, "ICON_LIST_CONTROL_COL_FILE", "file"}, {ICON_LIST_CONTROL_COL_PIXBUF, "ICON_LIST_CONTROL_COL_PIXBUF", "pixbuf"}, {ICON_LIST_CONTROL_COL_VALID, "ICON_LIST_CONTROL_COL_VALID", "valid"}, {0, NULL, NULL}};
		...
		case 0:
		{
			data->dir = g_file_new_for_path (data->dirname);
			{
				...
				if (!g_cancellable_is_cancelled (data->cancellable)) {
					...
					return FALSE;
					case 3:
					icon_list_control_retrieve_thumbnails_finish (data->self, data->_res_);
				}
				_g_object_unref0 (data->file_etor);
			}
			goto __finally1;
			...
			__finally1:
			{
				g_signal_emit_by_name (data->self, "done");
			}
			if (data->_inner_error_ != NULL) {
...
static void icon_list_control_retrieve_thumbnails_data_free (gpointer _data) {
	IconListControlRetrieveThumbnailsData* data;
	data = _data;
	_g_object_unref0 (data->cancellable);
	g_slice_free (IconListControlRetrieveThumbnailsData, data);
}


void icon_list_control_retrieve_thumbnails (IconListControl* self, GCancellable* cancellable, GAsyncReadyCallback _callback_, gpointer _user_data_) {
	IconListControlRetrieveThumbnailsData* _data_;
	_data_ = g_slice_new0 (IconListControlRetrieveThumbnailsData);
	_data_->_async_result = g_simple_async_result_new (G_OBJECT (self), _callback_, _user_data_, icon_list_control_retrieve_thumbnails);
	g_simple_async_result_set_op_res_gpointer (_data_->_async_result, _data_, icon_list_control_retrieve_thumbnails_data_free);
	_data_->self = self;
	_data_->cancellable = _g_object_ref0 (cancellable);
	icon_list_control_retrieve_thumbnails_co (_data_);
}
...
static void icon_list_control_retrieve_thumbnails_ready (GObject* source_object, GAsyncResult* _res_, gpointer _user_data_) {
	...
	data = _user_data_;
	data->_res_ = _res_;
	icon_list_control_retrieve_thumbnails_co (data);
}


static gboolean _icon_list_control_retrieve_thumbnails_co_gsource_func (gpointer self) {
	return icon_list_control_retrieve_thumbnails_co (self);
}
...
static void icon_list_control_class_init (IconListControlClass * klass) {
	...
	g_signal_new ("done", TYPE_ICON_LIST_CONTROL, G_SIGNAL_RUN_LAST, 0, NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
